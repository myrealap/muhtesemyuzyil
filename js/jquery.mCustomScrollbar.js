...
(function($){
	var methods={
			...
			return this.each(function(){
				...
				if(!$(document).data("mCS-is-touch-device")){ /*not touch device - apply custom scrollbars functionality*/
					...
					if(options.scrollButtons.enable){
						if(options.horizontalScroll){
							mCSB_scrollTools.prepend("<a class='mCSB_buttonLeft' style='display:block; position:absolute;'></a>").append("<a class='mCSB_buttonRight' style='display:block; position:absolute;'></a>");
						}else{
		...
		update:function(){
			$('.mCSB_buttonLeft').hide(); //cüneyt : baþlangýçta sol scroll un kapanmasý için
		...
		scroll:function(updated){
			...
			//$('.copy-in').html(draggerX + "-" + targX + "-" + thisX + "-" + posX + " - " + mCSB_container.width());
			//$('.copy-in').html($('.copy-in').html() + " : " + $this.find(".mCSB_dragger").width() + "-" + $this.find(".mCSB_container").width() + "-" + $this.find(".mCustomScrollBox").width());
			
				
				
			//cuneyt : sol ok'un kullanýlamayacaðý durum için kalkmasý
			//sag okun kalkmasý iþlemini onTotalScroll ve onScroll kýsýmlarýndan yapýlýr
			if (draggerX<=8){
